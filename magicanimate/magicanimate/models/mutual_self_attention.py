...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def __init__(
        self,
        total_steps=50,
        hijack_init_state=True,
        with_negative_guidance=False,
        appearance_control_alpha=0.5,
        mode='enqueue',
    ):
        """
    ...
    def mutual_self_attn(self, q, k, v, num_heads, **kwargs):
        ...
        out_tgt = self.attn_batch(
            q_tgt,
            torch.cat([k_tgt, k_src], dim=1),
            torch.cat([v_tgt, v_src], dim=1),
            num_heads,
            **kwargs,
        )
            ...
            for i, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
                ...
                if MODE == "read":
                    if len(self.mean_bank) > 0 and len(self.var_bank) > 0:
                        ...
                        hidden_states_c = hidden_states_uc.clone()
                        if do_classifier_free_guidance and style_fidelity > 0:
                            hidden_states_c[uc_mask] = hidden_states[uc_mask].to(
                                hidden_states_c.dtype
                            )
                        hidden_states = (
        ...
        if self.reference_adain:
            ...
            for r, w in zip(reader_gn_modules, writer_gn_modules):
                if len(w.mean_bank) > 0 and isinstance(w.mean_bank[0], list):
                    r.mean_bank = [
                        [v.clone().to(dtype) for v in vl] for vl in w.mean_bank
                    ]
                    r.var_bank = [
                        [v.clone().to(dtype) for v in vl] for vl in w.var_bank
                    ]
                else:

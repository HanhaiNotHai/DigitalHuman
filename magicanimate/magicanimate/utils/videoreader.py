...
class VideoReader:
    ...
    def _compute_video_stats(self):
        ...
        if num_of_frames == 0:
            num_of_frames = self.fps * float(
                self.container.streams.video[0].duration * self.video_stream.time_base
            )
    ...
    def sample(self, debug=False):
        ...
        offset = torch.randint(
            max(1, total_num_frames - self.num_frames - 1), [1]
        ).item()
        video_frames = self._read_video(offset / total_num_frames)
        video_frames = np.array(
            [np.uint8(f.to_rgb().to_ndarray()) for f in video_frames]
        )
        ...
        if self.bi_frame:
            ...
            frames = [
                int(frames[0] * self.num_frames),
                int(frames[1] * self.num_frames),
            ]
            frames.sort()
            video_frames = np.array(
                [video_frames[min(frames)], video_frames[max(frames)]]
            )
            Ts = [
                min(frames) / (self.num_frames - 1),
                max(frames) / (self.num_frames - 1),
            ]
            sample["frames"] = video_frames
